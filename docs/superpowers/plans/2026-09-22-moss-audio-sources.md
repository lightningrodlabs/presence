...
> **Status (2026-09-22): EXECUTED.** Moss branch `feat/audio-source-capture` off `main-0.7` @ `1c6db768`, commits `f66d4f53`..`09922dea` (16 commits: 7 task commits, 7 review-fix commits, the final-review fix wave of 3); merged `--no-ff` into `main-0.7` as `8a25037e` (2026-09-22; merged result green: 32 files / 327 tests, typecheck clean); the feature branch is deleted. Subagent-driven with an adversarial review per task (Tasks 3, 4, 5, 6 each needed one or two fix rounds; 1, 2, 7 clean) plus a whole-branch review (verdict "with fixes", fixed and re-reviewed). Gate at HEAD: `yarn test:unit` 32 files / 327 tests, `yarn typecheck` green. Round trip observed live (Task 8 Step 1, two agents): picker rows system-first with agent 2's process listed and agent 1's own tree absent; share → `{label:'System audio', canExcludeSelf:true}`, frames peak 26215 from `speaker-test`, counters after ~20 s `{chunksDropped:0, backlogDropped:20, stalls:0, recoveries:0, framesSent:2283}` (the backlog drops drove the pump catch-up fix); chip Stop → chip gone, list `[]`, port `{type:'ended', reason:'user-stopped'}`; cancel → `null`, 0 ports; WAL-window path not observed (example applet's committed `.happ` predates its zome source — pre-existing). Renamed 2026-09-23 on the Plan 3 branch (owner's ruling): the Settings tab "Capabilities" → "Services" (`TabsState.Services`, `<moss-services-settings>`, `self/settings/services/`; commit `138c8a3b`, in PR lightningrodlabs/moss#248); the task text below keeps the original names as the record of what was built. Deviations and the Plan 3/4 carry-overs are recorded in the spec's Section 2 "Landed" paragraph; every controller ruling is in the run's ledger (copy in the session scratchpad) and in the session's final message.
