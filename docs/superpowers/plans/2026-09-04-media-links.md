...
- Store keeps EXACTLY these surfaces: delegating getter `get _openConnections()` (`disconnect()` 2295 is the sole surviving store-side writer, keeps calling `.set` THROUGH the getter — verify it compiles and behaves, round-two screen-share precedent; `_clearPendingWebrtcStatus` 4267 is NOT an `_openConnections` writer — corrected 2026-09-04 post-Task-5-review, it writes `_connectionStatuses` and clears `pendingInits` only, an earlier version of this line mis-attributed it); bare delegates `_applyCloseCleanup(...)`, `_applyStaleTeardown(...)`, `_logSdpDataEvent(...)`, `_flushStaleSdpAggregates()` (callers: the ScreenShareLinks bindings at ~638/645, `handleLeaveUi`, the pong screen fragment, `pingAgents`, `handleSdpFsm` until Task 4). `start()` re-points `this._subscribeMediaTransport()` → `this.mediaLinks.subscribe()`.
...
- `_openConnections` writers through the delegating getter: confirm `disconnect()` (the sole surviving store-side writer) still hits the same `Writable` instance. (`_clearPendingWebrtcStatus` is not an `_openConnections` writer — corrected 2026-09-04 post-Task-5-review; see the amendment on decision 4 in the spec.)
