<!-- Copy of the extraction design spec, travelling with the package (decision 13). -->

> **Copy.** The authoritative original lives in the Presence monorepo at
> `docs/superpowers/specs/2026-09-08-signals-media-extraction-design.md` and stays
> authoritative until this package is extracted into its own repository, at which
> point this file becomes the original. Corrections go there first.
>
> Paths in the text below are relative to the Presence monorepo root, so
> `packages/signals-media/X` is `X` from here — the probe it cites as
> `packages/signals-media/docs/webkitgtk-probe/FINDINGS.md` is
> `docs/webkitgtk-probe/FINDINGS.md` in this package.
