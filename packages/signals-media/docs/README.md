...
| [`../API.md`](../API.md) | The API reference: one section per exported symbol of `src/index.ts` and of the `./opus-wasm` subpath — signature, contract, when it may be called, what it returns on failure. `../src/__tests__/api-doc.test.ts` fails when it and the barrel disagree. |
| [`../AGENTS.md`](../AGENTS.md) | The orientation file for modifying or embedding the package: the invariants and the file or test enforcing each, how to run the gates and what each proves, the per-platform plumbing an embedder owes, a map of `src/`, and what not to do. |
| [`../examples/README.md`](../examples/README.md) | Two host examples — a minimal one over a transport the reader supplies, and the full Holochain one over `@holochain/client` — typechecked by `tsconfig.examples.json`, plus the coordinator-zome half as reference-only Rust. |
