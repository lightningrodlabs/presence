...
format is byte-identical to Presence 0.15.6's, pinned by a golden fixture
(`src/__tests__/fixtures/wire.json`), so package peers and Presence peers
interoperate.
...
// recv_remote_signal(..) -> emit_signal(..)  // delivered to the sender's own UI socket
```

`msg_type` carries the `MediaKind` (`'voice'` / `'filmstrip'`) and `payload`
carries the library's string verbatim. On the client:

```ts
client.on('signal', signal => {
  const { from_agent, msg_type, payload } = signal.payload;
...
`src/__tests__/fixtures/wire.json` and asserted by `wire-fixture.test.ts`.
...
**In a source checkout these two helpers return empty strings.** The generated
sources are produced by `npm run build` and written into `dist/` — only the
built package carries them. Consuming the package from `src/` (a Vite alias to
`../src/index.ts`, say) gets an empty worker; alias `dist/` instead, as the
testbed does.
