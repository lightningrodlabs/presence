Status: SUPERSEDED (see `plans/local-models-asr.md` in the moss repo, `ai-transcription` branch; this copy is the 2026-04 snapshot Presence was written against)

# Moss `localModels` API — implementation plan

> **Status 2026-04-17:** this file is the snapshot we handed off to
> the Moss-side agent. The **living copy** is now in the Moss repo at
> `moss-ai-transcription/MOSS_LOCAL_MODELS_PLAN.md`, on branch
> `ai-transcription`, and has diverged with M0 spike findings
> (`smart-whisper` chosen, `latencyTier` added to capabilities,
> chunking-strategy notes in M1). Read that copy, not this one, for
> the current state. See
> `moss-ai-transcription/spikes/asr-m0/RESULTS.md` for the spike
> numbers.
>
> Keeping this file in the Presence repo so Presence-side agents have
> a self-contained record of what we originally specced and handed
> off. Do not update it unless you also propagate changes upstream.
...
### M1 — Moss-internal ASR service

- Wrap the runtime in a Moss-process service with a stable IPC
