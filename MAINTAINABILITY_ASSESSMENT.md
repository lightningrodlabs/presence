...
**Entry criterion (added 2026-07-28): Phase 6 items 1–2 land first** — the injectable clock and the construction/activation split. Phase 2's entire subject is clocks; extracting its decisions against ambient `Date.now`/`setTimeout` would reproduce the Phase 1 split — covered decisions, unverifiable wiring — at exactly the layer every production bug has lived in. See the Phase 6 correction below for why its own trigger will not fire in time.
...
**Correction, 2026-07-28 — the line-count arm is unreachable, and items 1–2 are promoted.** `streams-store.ts` *grew* across Phases 0.5–1, from 6,600 to 6,827 lines: extraction adds wiring and commentary to the god file faster than it deletes from it. Best-case planned deletions — ~235 carrier-aware lines (§3.5), Phase 4's 200–400, Phase 5's ~825 of forensics — land around 5,300–5,400, so the 3,500 trigger never fires from the planned work alone. Meanwhile every phase adds more wired-but-unasserted seams to the same file, which is the risk this phase exists to retire. Items 1 (the clock) and 2 (the construction/activation split) therefore run **before Phase 2** (see Phase 2's entry criterion); they are mechanical, and the blast radius is one construction site with one caller. Items 3–5 keep their original position behind the extraction phases.
