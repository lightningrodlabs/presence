...
| The screen-share FSM path has no harness coverage — `carrier-handover.spec.ts` exercises the media transport only; establishment, dir-routing, and mutual-share never run against a real RTCPeerConnection | `ui/harness/` | Phase 1.5 addendum (filed from the Phase 3 review, finding F4). Extend the nightly harness with a screen-share spec: sharer→viewer establishment over `SdpFsmScreen`, role-routing with both directions active, and slot teardown on `failed`. |
