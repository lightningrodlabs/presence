...
| The screen-share FSM path has no harness coverage — `carrier-handover.spec.ts` exercises the media transport only; establishment, dir-routing, and mutual-share never run against a real RTCPeerConnection | `ui/harness/` | **Scheduled as Phase 3.5** (filed from the Phase 3 review, finding F4; promoted from unscheduled 2026-07-29). The spec shape lives in that section. |
