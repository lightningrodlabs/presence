...
1. ~~Replace `carrierMode()`'s three-way collapse with two explicit axes (WebRTC on/off; signals always available as fallback).~~ **Substantively done by Phase 3**: with SimplePeer gone, `carrierMode()` collapsed to a two-value view over the one real axis (`webrtcGloballyDisabled`), and the UI selector is two-way. Remaining work is verify-and-declare only — confirm no third-mode residue in readers, keep or inline the getter, no new model. **Decision, 2026-07-29: no N-carrier selection machinery gets rebuilt for a hypothetical future carrier** (QUIC/WebTransport media was considered). The auto-flip deletion is the precedent: speculative executable selection policy rots (185 lines + 24 tests, never shipped its benefit). A future carrier's seams are the ones already exercised — `PeerTransport` (item 3 makes it a real annotation), a capability string + wire-contract row + corpus pin (absorbed `SdpFsmScreen` with zero structural change) — and its selection policy gets written as a fresh pure decision when its real requirements exist.
...
### Phase 5 — The diagnostic pipeline is a product; retire only what doesn't serve it (reframed 2026-07-29)
