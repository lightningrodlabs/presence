...
**Still outstanding — field validation.** None of the store wiring is verified at runtime. `StreamsStore` remains unconstructible under vitest (§3.6), so the extracted decisions are covered but their wiring is not. Validate with the live multi-agent VPN-flap test. Item 6 additionally needs a cross-version test: a v0.14.8 build against a v0.14.7 build, in both pubkey orderings — and per working agreement 7, a field log that disagrees with these 428 passing tests is the tests being wrong about reality, not the log.
