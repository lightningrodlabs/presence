...
**Still outstanding — field validation.** None of this is verified at runtime. `StreamsStore` remains unconstructible under vitest (§3.6), so the extracted decisions are covered but their *wiring* is not. Validate with the live multi-agent VPN-flap test. Item 6 additionally needs a cross-version test: a v0.14.8 build against a v0.14.7 build, in both pubkey orderings — and per working agreement 7, a field log that disagrees with these 421 passing tests is the tests being wrong about reality, not the log.
...
| ~~`conversation.ts:73-76` documents `simplepeer` winning the override tiebreaker; the code returns `fsm`~~ | `conversation.ts` | **Fixed in Phase 1** (`2c80400`), alongside item 6, which had to rewrite the same docblocks. The `peerImpl` comment's cited `auto-flip-policy.ts:149-152` had also drifted; it now names the function. |
